/* Zepto 1.2.0 - zepto ajax selector - zeptojs.com/license */
;(function(global, factory) {
    ...
    window.$Z === undefined && (window.$Z = Zepto)

    window.$Z.$select = function(selectors) {
        var result
        selectors.forEach(function(item) {
            if (result) {
                result = result[item.type].apply(result, item.params)
            } else {
                result = $Z.apply(window, item.params)
            }
        })
        return result
    }
    ...
    window.$Z.getMarks = function($elems) {
        const marks = []
        if (!$elems.length) return marks

        $elems.each(function(index) {
            if ($Z(this).attr('test-ppt-mark')) {
                marks.push($Z(this).attr('test-ppt-mark'))
            } else {
                marks.push(window.$Z.markUUID++)
                $Z(this).attr('test-ppt-mark', marks[marks.length - 1])
            }
        })
        return marks
    }
