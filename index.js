...
page = null;
// 没参数
const props0 = [ 'text', 'html', 'height', 'width', 'offset', 'offsetParent', 'position', 'val', 'index', 'scrollTop' ];
// 有参数
...
waitFors.response = async (urlSubstr, options) => {
	await page.waitForResponse((response) => response.url().indexOf(urlSubstr) > -1 && response.status() === 200);
};
...
expects.target = async (targetUrlSubstr, opened) => {
	...
	if ((typeof opened !== 'undefined' && res !== opened) || (typeof opened === 'undefined' && res)) {
		throw new Error(`expect target: ${targetUrlSubstr} ${opened ? 'opened' : 'closed'} but false`);
	}
};
...
class VSelector {
	constructor(selector) {
		...
		waitFors.visible = async (value, options) => {
			await waitFor(
				...
				options,
				`waiting for ${converToString(this.selectors)}.visible() to be '${value.toString()}' but timeout (#)`
			);
		};
		...
		waitFors.exist = async (value, options) => {
			await waitFor(
				...
				options,
				`waiting for ${converToString(this.selectors)}.exist() to be '${value.toString()}' but timeout (#)`
			);
		};
		...
		waitFors.length = async (value, options) => {
			await waitFor(
				...
				options,
				`waiting for ${converToString(this.selectors)}.length() to be '${value.toString()}' but timeout (#)`
			);
		};
			...
			expects[item] = async (value) => {
				if (!equat(await Domkit(this.selectors)[item](), value)) {
					throw new ExpectError(
						`expect ${converToString(this.selectors)}.${item}() to be '${value.toString()}' but false`
					);
				}
			};
		});
		...
		expects.visible = async (value) => {
			if (!equat(await this.visible(), value)) {
				throw new ExpectError(
					`expect ${converToString(this.selectors)}.visible() to be '${value.toString()}' but false`
				);
			}
		};
		...
		expects.exist = async (value) => {
			if (!equat(await this.exist(), value)) {
				throw new ExpectError(
					`expect ${converToString(this.selectors)}.exist() to be '${value.toString()}' but false`
				);
			}
		};
		...
		expects.length = async (value) => {
			if (!equat(await this.length(), value)) {
				throw new ExpectError(
					`expect ${converToString(this.selectors)}.length() to be '${value.toString()}' but false`
				);
			}
		};
     ...
     */
	async click(opts) {
		this.domSelector = await converToDomSelector(
			assignSelectors(this.selectors, [ { type: 'eq', params: [ 0 ] } ])
		);
...
selectors.forEach((item) => {
	VSelector.prototype[item] = function() {
		return new VSelector(
			assignSelectors(this.selectors, [ { type: item, params: Array.prototype.slice.call(arguments) } ])
		);
	};
});
